...
const NAME_NOT_FOUND =
  "Không tìm thấy tên Facebook này trong danh sách lớp. Vui lòng nhập đầy đủ và đúng tên Facebook của bạn.";
...
const signCandidate = (member) => {
  ...
  return jwt.sign(
    { memberId: member._id.toString(), purpose: "reunion-registration" },
...
const signMember = (member) => {
  if (!process.env.JWT_SECRET) throw new Error("JWT_SECRET is not configured");
  return jwt.sign(
    {
      memberId: member._id.toString(),
      scope: "reunion:member",
    },
    process.env.JWT_SECRET,
    {
      expiresIn: "90d",
...
const resolveMember = async ({ facebookName, candidateToken }) => {
  if (candidateToken) {
    const payload = jwt.verify(candidateToken, process.env.JWT_SECRET, {
      issuer: "wedding-api",
      audience: "reunion-candidate",
    });
    if (payload.purpose !== "reunion-registration")
  ...
  const matches = await ClassMember.find({ normalizedFullName: normalized })
    .sort({ seedKey: 1 })
    .lean();
  ...
  if (matches.length > 1)
    return {
      candidates: matches.map((member) => ({
        ...safeMember(member),
        candidateToken: signCandidate(member),
      })),
    };
  return matches[0];
};
  ...
  try {
    ...
    if (!Number.isInteger(count) || count < 1 || count > 50) {
      return res
        ...
        .json({
          success: false,
          message: "Số người tham gia phải là số nguyên từ 1 đến 50.",
        });
    }
    ...
    if (resolved?.validationError)
      return res
        .status(400)
        .json({ success: false, message: resolved.validationError });
    if (resolved?.candidates)
      return res
        ...
        .json({
          ...
          code: "AMBIGUOUS_NAME",
          message:
            "Có nhiều thành viên cùng tên. Vui lòng chọn đúng tài khoản của bạn.",
          candidates: resolved.candidates,
        });
    if (!resolved)
      return res.status(400).json({ success: false, message: NAME_NOT_FOUND });
    ...
    if (existing) {
      return res.status(200).json({
        ...
        data: {
          ...
          memberToken: signMember(existing.memberId),
        },
      });
    }
    ...
    try {
      ...
      return res.status(201).json({
        success: true,
        data: {
          ...serializeRegistration(populated),
          memberToken: signMember(populated.memberId),
        },
      });
    } catch (error) {
      if (error?.code === 11000) {
    ...
    if (
      ...
      return res
        ...
        .json({
          success: false,
          message:
            "Lựa chọn thành viên đã hết hạn. Vui lòng nhập lại tên Facebook.",
        });
    }
...
const adminLogin = async (req, res) => {
  ...
  if (!configured)
    return res
      .status(503)
      .json({ success: false, message: "ADMIN_PIN chưa được cấu hình." });
  const valid =
    pin.length === configured.length &&
    crypto.timingSafeEqual(Buffer.from(pin), Buffer.from(configured));
  if (!valid)
    return res
      .status(401)
      .json({ success: false, message: "Mã quản trị không chính xác." });
  if (!process.env.JWT_SECRET)
    return res
      .status(503)
      .json({ success: false, message: "JWT_SECRET chưa được cấu hình." });
  const token = jwt.sign({ scope: "reunion:admin" }, process.env.JWT_SECRET, {
    expiresIn: "8h",
    issuer: "wedding-api",
...
const adminStatus = (req, res) => {
  try {
    ...
    return res.json({
      success: true,
      data: {
        isAdmin: payload.scope === "reunion:admin",
        amountEntryIsAvailable: reunionConfig.amountEntryIsAvailable(),
      },
    });
  } catch (_error) {
...
const findRegistration = async (id) =>
  mongoose.isValidObjectId(id) ? Registration.findById(id) : null;
...
const updatePaymentStatus = async (req, res) => {
  if (typeof req.body?.isPaid !== "boolean")
    return res
      .status(400)
      .json({ success: false, message: "Trạng thái kinh phí không hợp lệ." });
  const item = await findRegistration(req.params.id);
  if (!item)
    return res
      .status(404)
...
const updateAmount = async (req, res) => {
  if (!reunionConfig.amountEntryIsAvailable())
    return res
      .status(403)
      .json({
        success: false,
        message:
          "Chức năng nhập số tiền chỉ mở sau hạn đóng kinh phí 22/08/2026.",
      });
  ...
  if (!Number.isFinite(amount) || amount < 0)
    return res
      ...
      .json({
        success: false,
        message: "Số tiền đã nhận phải là số không âm.",
      });
  const item = await findRegistration(req.params.id);
  if (!item)
    return res
